...
\comment{while we don't attempt to recognize what kind of scene the robot is looking at
  from some large set, we do think that it's important to know if a segmentable trail is present or not in the image.  so we talk about classifying the image in this way as a pre-processing step.}
...
To create a train/test dataset, images were selected from multiple
...
The total number of logged images was 13,179, of which 1,302 were
marked as \textit{bad}.  Trail shapes were annotated for 553 images
(at 20-frame intervals from the captured sequences), and 256
representative bad trail images were selected.
...
high-valued state, then the system infers that there is no clear trail
...
images.  Specifically, the color trail ROI is resized to $160 \times
80$ and input to a Darknet-19 convolutional neural network
\cite{<YOLO9000>}, which is used as a feature-finding front end.  This
is followed by several fully-connected layers \comment{number of
  layers, number of units} before a softmax output layer.
...
\comment{define terms like IoU.  training/testing splits on various
  datasets.  ``old'' particle filtering approach with likelihood
  function: how we ran it on test images and got linear trail
  segmentations.  IoU metrics: mean/median/min/max.  how fast?}

For training/testing, the images annotated with trail shape were split 80\%/20\% to get
1614 training images and 404 test images.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Trail quality}

\comment{Training set with distortions.  Test set.  Learning rate,
  number of iterations.  Darknet-19 quality network's percent correct,
  precision and recall at threshold = 0.5 (not worth showing P-R curve
  until/unless there are multiple comparisons to make).  Average
  precision over all thresholds.}
...
%  \centering{\includegraphics[width=0.8\columnwidth]{quality_net_PR_curve.png}}}
  \centering{\includegraphics[width=1.0\columnwidth]{sep08_bad_trail_pr_curves.png}}}
\caption{Precision-recall curves on test, validation sets of different methods for assessing whether image contains a trail region or is a \textit{bad trail}}
...
1,614 training images and 404 test images \comment{what about that one
  with the trail outside image in sequence?}
...
\caption{Success plot of \comment{different} trail tracking methods on $\TrailSeqAnnotatedSet^{test}$}
\label{fig:SP1_test}
\end{figure}
